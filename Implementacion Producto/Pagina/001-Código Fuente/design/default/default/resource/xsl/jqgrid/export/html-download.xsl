...
<!--<xsl:value-of select="php:function('header','content-disposition:attachment;filename=bla.html')" />
-->
<xsl:value-of select="php:function('Core_Http_Header::ContentDisposition',concat(//rows/params/title,'.html'))" />
  <html>
  <head>
  <style>
  <![CDATA[
