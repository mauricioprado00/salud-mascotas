...
class Frontend_Mascota_Encuentro_Listado_Router extends Frontend_Router_Abstract{
	...
	protected function mascota($id_mascota){
		$ids_mascotas_encuentros = $this->getHelper()->getIdsMascotasEncuentros(true, $id_mascota);
		return $this->pagina(0);
	}
	protected function pagina($numero_pag=0){
		$ids_mascotas_encuentros = $this->getHelper()->getIdsMascotasEncuentros();
		$ids_domicilios_encuentros = $this->getHelper()->getIdsDomiciliosEncuentros();
...
//		die(__FILE__.__LINE__);
		$map_swicher = $loaded_layout->getBlock('map_swicher')
			...
			->setDomicilios($this->getHelper()->prepareDomiciliosEncuentros(intval($numero_pag)))
