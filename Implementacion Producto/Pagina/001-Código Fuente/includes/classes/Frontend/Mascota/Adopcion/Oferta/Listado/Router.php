<?php /*es útf8*/
class Frontend_Mascota_Adopcion_Oferta_Listado_Router extends Frontend_Router_Abstract{
	...
	protected function mascota($id_mascota){
		$ids_mascotas_adopcion_ofertas = $this->getHelper()->getIdsMascotasAdopcionOfertas(true, $id_mascota);
		return $this->pagina(0);
	}
	protected function pagina($numero_pag=0){
		$ids_mascotas_adopcion_ofertas = $this->getHelper()->getIdsMascotasAdopcionOfertas();
		$ids_domicilios_adopcion_ofertas = $this->getHelper()->getIdsDomiciliosAdopcionOfertas();
		$id_mascota = $ids_mascotas_adopcion_ofertas[$numero_pag];
		$id_domicilio = $ids_domicilios_adopcion_ofertas[$numero_pag];
		...
		$domicilio->load();
//		var_dump($id_domicilio,$ids_domicilios_adopcion_ofertas,$domicilio->getData());
//		die(__FILE__.__LINE__);
		//var_dump($ids_domicilios_adopcion_ofertas);
		Core_App::getInstance()->setPagina($numero_pag);
		$this->setPageReference('Mascotas', 'AdopcionOfertas');
		Core_App::getLayout()
			->setModo('saludmascotas')
			->addAction('mascotas_adopcion_ofertas')
		;
...
//		die(__FILE__.__LINE__);
		$map_swicher = $loaded_layout->getBlock('map_swicher')
			...
			->setDomicilios($this->getHelper()->prepareDomiciliosAdopcionOfertas(intval($numero_pag)))
...
//		;
		$this->setActiveLeftMenu('dashboard_mascotas_adopcion_ofertas');
	}
