<?php /*es útf8*/
class Frontend_Mascota_Adopcion_Oferta_Router extends Frontend_Mascota_Router{
...
//	}
	...
	protected function _pre_editar_init($paso=1, $id_mascota=null, $preserve_mascota_edicion=false){
		...
		$adopcion_oferta = null;
		if(!$preserve_mascota_edicion || !($adopcion_oferta = Frontend_Mascota_Adopcion_Oferta_Helper::getAdopcionOfertaEdicionFromSession($id_mascota))){
			if(in_array($paso, array(2,4))||!$preserve_mascota_edicion)
				$adopcion_oferta = Frontend_Mascota_Adopcion_Oferta_Helper::getAdopcionOfertaEdicion($object_to_edit);
			//$adopcion_oferta = $object_to_edit->getAdopcionOferta();
		}
		if(!$adopcion_oferta)
			$adopcion_oferta = new Frontend_Model_AdopcionOferta();
		
		if($preserve_mascota_edicion){
			$coincidencias_seleccionadas = Frontend_Mascota_Adopcion_Oferta_Helper::getCoincidenciasSeleccionadasFromSession($id_mascota);
			if(!isset($coincidencias_seleccionadas)){
				$coincidencias_seleccionadas = $adopcion_oferta->getIdsCoincidenciasSeleccionadas();
			}
		}
		else{
			$coincidencias_seleccionadas = $adopcion_oferta->getIdsCoincidenciasSeleccionadas();
			//var_dump($coincidencias_seleccionadas); die(__FILE__.__LINE__);
		}
		
		if(!isset($coincidencias_seleccionadas)){
			$coincidencias_seleccionadas = array();
		}
//		echo Core_Helper::DebugVars($adopcion_oferta->getData());
//		die(__FILE__.__LINE__);
		...
		switch($paso){
			...
			case 3:{
				...
				$adopcion_oferta->commitNonTableColumn();
				//var_dump(get_class($adopcion_oferta));die();
				//var_dump($object_to_edit->getIdEspecie());die(__FILE__.__LINE__);
				$coincidencias = $adopcion_oferta->getCoincidencias();
			...
			case 4:{
				//incluyo adopcion_solicituds seleccionados actualmente y en otros registros
				$ids_coincidencias = $adopcion_oferta->getIdsCoincidenciasSeleccionadas();
...
//				die(__FILE__.__LINE__);
				if($ids_coincidencias)
					$coincidencias = $adopcion_oferta->getCoincidencias($ids_coincidencias);
		...
		$this->setAdopcionOferta($adopcion_oferta);
	...
	protected function _editar_step_ok($paso=1, $id_mascota=null, $preserve_mascota_edicion=false){
		switch($paso){
			case 1:{//si el paso está bien cargado redirijo al paso siguiente
				$object_to_edit = $this->getObjectToEdit();
				Core_Http_Header::Redirect(Frontend_Mascota_Adopcion_Oferta_Helper::getUrlEditar($object_to_edit->getId(), 1, 2), true);
				return true;
				break;
			}
			case 2:{//si el paso esta bien cargado (domicilio) checkeo la carga de fecha y hora de extravio y redirijo al paso siguiente
				Core_Http_Header::Redirect(Frontend_Mascota_Adopcion_Oferta_Helper::getUrlEditar($id_mascota, 1, 3), true);
				return true;
//				$adopcion_oferta = $this->getAdopcionOferta();
//				$post = Core_Http_Post::getParameters('Core_Object', Frontend_Mascota_Adopcion_Oferta_Helper::getUpdatableAdopcionOfertaFields());
//				$adopcion_oferta->loadFromArray($post->getData(), false);
////				header('content-type:text/plain');
////				var_dump($adopcion_oferta->getData());
////				die(__FILE__.__LINE__);
//				$object_to_edit = $this->getObjectToEdit();
//				$id_mascota = $object_to_edit->getId();
//				$guardado_en_sesion = Frontend_Mascota_Adopcion_Oferta_Helper::actionAgregarEditarAdopcionOferta($adopcion_oferta, true, $id_mascota)?true:false;
...
//				}
			...
			case 3:{//si las selecciones de mascotas coincidentes en la búsqueda estan correctas redirijo al paso siguiente
				$object_to_edit = $this->getObjectToEdit();
				Core_Http_Header::Redirect(Frontend_Mascota_Adopcion_Oferta_Helper::getUrlEditar($object_to_edit->getId(), 1, 4), true);
				break;
			}
...
//var_dump($id_mascota);die(__FILE__.__LINE__);
						$id_mascota = $object_to_edit->getId();
						$guardado = Frontend_Mascota_Adopcion_Oferta_Helper::actionAgregarEditarAdopcionOferta($adopcion_oferta, false, $id_mascota, $domicilio_mascota)?true:false;
						if($guardado){//pasa validaciones
							$guardadas = Frontend_Mascota_Adopcion_Oferta_Helper::crearAdopcionConciliacions($adopcion_oferta, $this->getCoincidenciasSeleccionadas())?true:false;
							if($guardadas){
								...
								if($object_to_edit->hasAdopcionConciliacion())
									...
									Core_Http_Header::Redirect(Frontend_Mascota_Adopcion_Solicitud_Helper::getUrlUsuario(), true);
								return true;
							}
						}
//						$this->getHelper()->clearSessionVars();
...
//				$id_mascota = $object_to_edit->getId();
//				$domicilio_mascota = $this->getDomicilioMascota();
//				$guardado_en_sesion = Frontend_Mascota_Adopcion_Oferta_Helper::actionAgregarEditarAdopcionOferta($adopcion_oferta, false, $id_mascota, $domicilio_mascota)?true:false;
//				if($guardado_en_sesion){//pasa validaciones
//					$object_to_edit = $this->getObjectToEdit();
//					Core_Http_Header::Redirect(Frontend_Mascota_Adopcion_Oferta_Helper::getUrlUsuario(), true);
//					return true;
//				}
//				die('guardo domicilio, mascota, adopcion_conciliacions y anuncio '.__FILE__.__LINE__);
//				Core_App::getInstance()->addErrorMessage('guardo domicilio, mascota, adopcion_conciliacions y anuncio '.__FILE__.__LINE__);
				break;
			}
		}
	}
	...
	protected function _pre_editar_handle_post($paso=1, $id_mascota=null, $preserve_mascota_edicion=false){
		...
		switch($paso){
			...
			case 2:{
				$adopcion_oferta = $this->getAdopcionOferta();
				$post = Core_Http_Post::getParameters('Core_Object', Frontend_Mascota_Adopcion_Oferta_Helper::getUpdatableAdopcionOfertaFields());
				$adopcion_oferta->loadFromArray($post->getData(), false, true);
//				header('content-type:text/plain');
//				var_dump($adopcion_oferta->getData());
...
//				die(__FILE__.__LINE__);
			...
			case 3:{//verificar que las selecciones sean correcta y guardar en sesión
				...
				$guardado = Frontend_Mascota_Adopcion_Oferta_Helper::setCoincidenciasSeleccionadasInSession($coincidencias_seleccionadas);
				if($guardado){
					return true;
...
//			;
		...
		if($paso==4){
			...
			$view_datos_mascota = $loaded_layout
				...
				//->addExtraData('Fecha Extravío', $adopcion_oferta->getExtravioFecha())
			;
			$view_ubicacion = $loaded_layout
				->getBlock('view_ubicacion')
				->setDomicilio($this->getDomicilioMascota())
			;
//			var_dump($this->getDomicilioMascota()->getData());
//			die(__FILE__.__LINE__);
...
//		$this->_editar_handle_setActiveLeftMenu($paso, $id_mascota, $preserve_mascota_edicion);
	...
	protected function _editar_handle_init_layout($paso=1, $id_mascota=null, $preserve_mascota_edicion=false){
		$this->setPageReference('Ofrecer mascota', 'Publicar anuncio');
		...
		Core_App::getLayout()
			->setModo('saludmascotas')
			->addAction('mascota_adopcion_oferta_addedit')
			->addAction('mascota_adopcion_oferta_addedit_step_'.$paso)
			->addAction('mascota_adopcion_oferta_addedit_' . $handle);
		;
		$this->showLeftMenu('usuario');	
	}
...
//			return $return;
//	}
	protected function automatica($id_mascota){
		Frontend_Mascota_Helper::setParaAdoptar($id_mascota);
		Frontend_Mascota_Adopcion_Oferta_Helper::actionAgregarAdopcionOfertaAutomatico($id_mascota);
//		die(__FILE__.__LINE__);
		return $this->redirect(Core_Http_Header::getRequest('referer'), false);
	}
}
?>