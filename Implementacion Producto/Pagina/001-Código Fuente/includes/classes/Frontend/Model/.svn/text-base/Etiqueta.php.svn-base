<?php //es útf8
/**
 *@referencia Parent(id_parent) Frontend_Model_Etiqueta(id)
 *@referencia Usuario(id_usuario) Frontend_Model_Usuario(id)
*/
class Frontend_Model_Etiqueta extends Saludmascotas_Model_Etiqueta{
...
//			->addValidator('entrenada', c(new Zend_Validate_NotEmpty(array('allowWhiteSpace' => true))))
			...
			->addValidator('nombre', c(new Zend_Validate_Alnum(array('allowWhiteSpace' => true))))
...
//		$this->setFechaExpiracion(time()+60*60*24*Saludmascotas_Model_Config::findConfigValue('sm/dias_expiracion_encuentro', 7*4/*4 semanas*/));
	...
	public function getUrlEliminar(){
		return Frontend_Etiqueta_Helper::getUrlEliminarEtiqueta($this->getId());
	}
}
