<?php //es útf8
/**
 *@referencia Spa(id_spa) Frontend_Usuario_Model_User(id)
*/
class Frontend_Model_Patrullaje extends Saludmascotas_Model_Patrullaje{
...
//			->addValidator('entrenada', c(new Zend_Validate_NotEmpty(array('allowWhiteSpace' => true))))
			...
			->setFieldLabel('fecha','Fecha de Patrullaje')
	...
	public function loadNonTableColumn(){
		...
		//$time = $this->getHoraPatrullaje();
		//var_dump($time);
//		$time = explode(' ', $time);
//		$this
//			->setData('encuentro_fecha', $time[0], array())
//			->setPatrullajeHora($time[1])
//		;
		return $this;
	}
	public function commitNonTableColumn(){
//		$hora_encuentro = $this->getData('encuentro_fecha', null, array());
//		$hora_encuentro .= ' ' . $this->getPatrullajeHora();
//		$this->setHoraPatrullaje($hora_encuentro, array());
	}
