<?
class Admin_ProyectoActividad_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		$proyecto_actividad = new Inta_Model_ProyectoActividad();
		if($comparator!=null){
			$proyecto_actividad->setWhere($comparator);
		}
		$datos = array();
		$total_items = $proyecto_actividad->searchCount();
...
//		}
		...
		//$datos = $proyecto_actividad->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso'));
		$datos = $proyecto_actividad->search($sidx,$sord,$rows,$rows*($page-1),get_class($proyecto_actividad));
