<?
class Admin_Aspecto_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		$aspecto = new Inta_Model_Aspecto();
		if($comparator!=null){
			$aspecto->setWhere($comparator);
		}
		$datos = array();
		$total_items = $aspecto->searchCount();
...
//		}
		...
		//$datos = $aspecto->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso'));
		$datos = $aspecto->search($sidx,$sord,$rows,$rows*($page-1),get_class($aspecto));
