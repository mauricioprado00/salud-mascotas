<?php
class Admin_Reporte_Helper_Export_ExcelPorProyectoPorAgenciaDetallado extends Core_Singleton{
	const USE_SHELL_EXEC = true;
	const DOWNLOAD_FILE_NAME = 'Actividades_x_Proyecto_x_Agencia.xml';
	const TITULO = 'Listado de Actividad por Agencia por cada Proyecto';
...
//			$reporte = new Inta_Model_Reporte_Actividad();
			$reporte = new Inta_Model_Reporte_View_ActividadByProyecto(false);
			$reporte->setWhere(Db_Helper::equal('id_usuario_logeado', 4));
			$actividades = $reporte->search('nombre_agencia, nombre_proyecto', 'ASC', null, 0, 'Inta_Model_Reporte_Actividad');
//			echo $reporte->searchGetSql('nombre_agencia, nombre_proyecto', 'ASC', null, 0, 'Inta_Model_Reporte_Actividad');
//			die();
			...
			<model for="resultado_actividad">
				...
				<method name="actividad_instancia" method="getActividad" multiplicity="single">
					<model for="actividad">
						...
						<method name="nombres_proyectos" method="getNombresProyectos" multiplicity="single" />
					</model>
				</method>
				<!-- 
...
//		Core_Http_Header::ContentType('text/xml');echo $g->toXmlString($datamodel);die();

		$xs = new Core_Xslt_Server();
		$xs->setSource($g, $datamodel);
//		$xs->appendStyle(dirname(__FILE__).'/resource/reporte3toflat.xsl');
		$xs->appendStyle(dirname(__FILE__).'/resource/reporte5toflat.xsl');
