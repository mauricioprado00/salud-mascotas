<?
class Admin_AudienciaActividad_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		$audiencia_actividad = new Inta_Model_AudienciaActividad();
		if($comparator!=null){
			$audiencia_actividad->setWhere($comparator);
		}
		$datos = array();
		$total_items = $audiencia_actividad->searchCount();
...
//		}
		...
		//$datos = $audiencia_actividad->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso'));
		$datos = $audiencia_actividad->search($sidx,$sord,$rows,$rows*($page-1),get_class($audiencia_actividad));
