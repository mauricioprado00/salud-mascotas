<?
class Admin_Translate_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		$datos = array();
		$total_items = $translate->searchCount();
...
//		}
		...
		//$datos = $translate->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso'));
		$datos = $translate->search($sidx,$sord,$rows,$rows*($page-1));
