<?
class Admin_Estrategia_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		$estrategia = new Inta_Model_Estrategia();
		if($comparator!=null){
			$estrategia->setWhere($comparator);
		}
		$datos = array();
		$total_items = $estrategia->searchCount();
...
//		}
		...
		//$datos = $estrategia->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso'));
		$datos = $estrategia->search($sidx,$sord,$rows,$rows*($page-1),get_class($estrategia));
