<?
class Admin_Indicador_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		$indicador = new Inta_Model_Indicador();
		if($comparator!=null){
			$indicador->setWhere($comparator);
		}
		$datos = array();
		$total_items = $indicador->searchCount();
...
//		}
		...
		//$datos = $indicador->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso'));
		$datos = $indicador->search($sidx,$sord,$rows,$rows*($page-1),get_class($indicador));
