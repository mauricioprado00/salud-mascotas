<?
class Admin_Proyecto_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		if($comparator!=null){
			$proyecto->setWhere($comparator);
		}
		$datos = array();
		$total_items = $proyecto->searchCount();
...
//		}
		...
		//$datos = $proyecto->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso'));
		$datos = $proyecto->search($sidx,$sord,$rows,$rows*($page-1));
