<?
class Admin_ResultadoEsperadoActividad_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		$resultado_esperado_actividad = new Inta_Model_ResultadoEsperadoActividad();
		if($comparator!=null){
			$resultado_esperado_actividad->setWhere($comparator);
		}
		$datos = array();
		$total_items = $resultado_esperado_actividad->searchCount();
...
//		}
		...
		//$datos = $resultado_esperado_actividad->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso'));
		$datos = $resultado_esperado_actividad->search($sidx,$sord,$rows,$rows*($page-1),get_class($resultado_esperado_actividad));
