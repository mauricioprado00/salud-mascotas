<?
class Admin_TipoAudiencia_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		if($comparator!=null){
			$tipo_audiencia->setWhere($comparator);
		}
		$datos = array();
		$total_items = $tipo_audiencia->searchCount();
...
//		}
		...
		//$datos = $tipo_audiencia->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso'));
		$datos = $tipo_audiencia->search($sidx,$sord,$rows,$rows*($page-1));
