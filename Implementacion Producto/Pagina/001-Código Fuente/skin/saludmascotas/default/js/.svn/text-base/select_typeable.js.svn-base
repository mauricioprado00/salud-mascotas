...
//			})
			...
			this.jqcontrol.mouseleave(this.handle_control_moseleave.bindAsEventListener(this));
			this.jqcontrol.mouseenter(this.handle_control_mouseenter.bindAsEventListener(this));
		...
		hide_options: function(){
			this.jqcontrol
				.removeClass('select_typeable_downed')
			//this.jqoptions.hide();
		},
		handle_control_moseleave: function(e){
//			window.console.log(e.target,jQuery(e.target).hasClass('select_typeable'), jQuery(e.target).attr('class'));
//			if(!jQuery(e.target).hasClass('select_typeable'))
//				return;
//			window.console.log(e.target,'blur options');
//			window.console.log(this.jqcontrol);
			this.jqcontrol
				.removeClass('select_typeable_downed')
				.removeClass('select_typeable_recently_loaded')
			;
		},
		handle_control_mouseenter: function(e){
			if(this.jqcontrol.hasClass('select_typeable_recently_loaded')){
				this.jqcontrol.addClass('select_typeable_downed');
				this.jqcontrol.removeClass('select_typeable_recently_loaded');
			}
			return;
//			window.console.log(e.target,jQuery(e.target).hasClass('select_typeable'), jQuery(e.target).attr('class'));
//			if(!jQuery(e.target).hasClass('select_typeable'))
...
//			window.console.log('doing search');
		...
		on_search_return: function(data){
			...
			if(data.resultados.length){
				...
				this.jqcontrol.addClass('select_typeable_recently_loaded')
				setTimeout(function(){
					if(!(--that.recently_loaded))
						that.jqcontrol.removeClass('select_typeable_recently_loaded');
				}, 5000)
				//this.show_options();
			}
